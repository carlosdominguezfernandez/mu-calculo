...
class BDDMapping:
    ...
    def __init__(self, apta: APTA, aut):
        ...
        self.state_length = self._representation_length(len(self.modal_ex_states))
    ...
    @staticmethod
    def _representation_length(n: int):
        """How many bits are needed to represent n different values"""

        return (n - 1).bit_length() if n >= 2 else n
    ...
    def translate_back_label(self, bdd: buddy.bdd, bdict):
        ...
        f = spot.bdd_to_formula(bdd, bdict)

        # Clauses of the disjunctive normal form
        clauses = list(f) if f.kind() == spot.op_Or else (f,)

        # Each clause is a separate edge
        for clause in clauses:
            ...
            for literal in literals:
                match literal.kind():
                    ...
                    case _:
                        raise ValueError(f'unexpected subformula in a CNF: {f} -> {literal}')
            ...
            # The second coordinate is a modal state
                ...
                if '_u0' not in values:
                    ...
                    # Translate the existential state back from the encoding
                    state_nr = self.modal_ex_states[state_nr] \
                        if state_nr < len(self.modal_ex_states) \
                        else len(self.modal_ex_states)

                yield Label(Label.Type.STATE, extra=state_nr, aprops=aprops)


def _normalize_initial(initial: int, state_nr: int):
    """Normalize the initial state to zero"""

    return 0 if state_nr == initial else (initial if state_nr == 0 else state_nr)
...
def determinize(automaton: NPA):
    ...
    daut = aut.postprocess('deterministic', 'parity max even', 'colored', 'sbacc')

    # Initial state (it need not be 0)
    initial_state = daut.get_init_state_number()
    ...
    for edge in daut.edges():
        src_state = new_aut.states[_normalize_initial(initial_state, edge.src)]
        ...
        for label in bddm.translate_back_label(edge.cond, bdict):
            src_state.next.setdefault(label, []).append(_normalize_initial(initial_state, edge.dst))
